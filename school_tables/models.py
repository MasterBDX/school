...
class TheClass(models.Model):
    ...
    def get_mid_exams(self):
        return self.exams_table.filter(exam_type=1)

    def get_final_exams(self):
        return self.exams_table.exclude(exam_type=1)
...
class ExamTable(models.Model):
    ...
    the_class = models.ForeignKey(
        TheClass, related_name='exams_table', on_delete=models.CASCADE)
...
class Exam(models.Model):
    exam_table = models.ForeignKey(ExamTable, on_delete=models.CASCADE,
                                   related_name='exams',
    ...
    def __str__(self):
        return self.subject + ' ' + ' إمتحان ' + self.exam_table.__str__()
